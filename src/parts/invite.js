...
export default function Invite(props) {
    ...
    const updateResponse = (form) => {
        ...
        if (form.willAttend) {
            setContent(
                "Cảm ơn vì đã gửi phản hồi cho chúng tôi! Chúng tôi sẽ mong chờ sự hiện diện của bạn tại bữa tiệc!"
            );
        } else {
            setContent(
                "Cảm ơn vì đã gửi phản hồi cho chúng tôi! Rất tiếc vì bạn đã không thể tham dự được!"
            );
        }
    };
    ...
    return (
        <div
            ...
            {hasResponse ? (
                <p
                    className="bodyText"
                    style={{ ...pStyle, width: "75vw", maxWidth: "500px" }}>
                    {content}
                </p>
            ) : (
                <div>
                    <p
                        ...
                        để thuận tiện cho việc sắp xếp chỗ ngồi, vui lòng phản
                        hồi giúp gia đình chúng tôi nhé!
                    </p>
                    <ResponseForm
                        updateResponse={updateResponse}
                        formData={props.invitation}
                    />
                </div>
            )}
        </div>
...
export function ResponseForm(props) {
    ...
    useEffect(() => {
        if (
            !window.location.href.endsWith("/secret/admin") &&
        ...
        if (formData.name == "" || !formData.name) {
            ...
            try {
                ...
                if (props.formData.id) {
                    console.log("edit");
                    await firestoreService.editReply(
                        props.formData.id,
                        formData
                    );
                    if (props.close) {
                ...
                <>
                    ...
                    <Grid item xs={6}>
                        <FormControl variant="standard" fullWidth>
                            <InputLabel id="item-select-label-form">
                                Số người tham dự
                            </InputLabel>
                            <Select
                                id="form-attendees"
                    ...
                    <Grid item xs={6}>
                        <FormControl variant="standard" fullWidth>
                            <InputLabel id="transport-select-label-form">
